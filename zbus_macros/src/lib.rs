...
///
/// The method and property arguments support the following `zbus` attributes:
///
...
///   which will be `None` for getters when the function is being called as part of the initial
///   object setup (before it gets registered on the bus), or when we send out property changed
///   notifications.
