...
/// see [`Connection::graceful_shutdown`]. To immediately close a connection in a way that will
/// disrupt any outstanding method calls, see [`Connection::close`].
///
...
impl Connection {
    ...
    /// # Caveats
    ///
    /// * Calling this will not prevent new incoming messages from keeping the collection alive, so
    ///   you may want to take precautions like deregistering well-known names to reduce the
    ///   likelihood of this occurring.
    ///
    /// * This shutdown will not complete until the underlying connection is fully dropped, so
    ///   beware of deadlocks if you are holding any other clones of this `Connection`.
    ///
    /// # Example
    ///
    /// ```
    ...
    /// }
    ///
    /// async fn graceful_example() -> Result<(), Box<dyn Error>> {
    ///     let conn = Builder::session()?
    ///         .name("foo.bar.baz")?
    ...
    ///         .await?;
    ///
    ///     // Wait for exit condition here.
    ///     // some_exit_condition.await;
    ///
    ///     conn.release_name("foo.bar.baz").await?;
