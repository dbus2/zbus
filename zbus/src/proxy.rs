...
pub struct PropertyChanged<'a, T> {
    name: &'a str,
    interface_name: InterfaceName<'a>,
    properties: Arc<PropertiesCache>,
    properties_proxy: PropertiesProxy<'a>,
...
impl<'a> Proxy<'a> {
    ...
    pub async fn receive_property_changed<'p, T>(&'p self, name: &'p str) -> PropertyStream<'p, T> {
        let properties_proxy = self.properties_proxy();
        let interface_name = self.inner.interface.clone();
