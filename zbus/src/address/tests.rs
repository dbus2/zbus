...
fn parse_err() {
    assert_eq!(
        Address::try_from("").unwrap_err().to_string(),
        "Missing transport in address"
    );
    assert_eq!(
        Address::try_from("foo").unwrap_err().to_string(),
        "Missing transport in address"
    );
    assert_eq!(
        Address::try_from("foo:").unwrap_err().to_string(),
    ...
    assert_eq!(
        Address::try_from("tcp:opt=%1").unwrap_err().to_string(),
        "Encoding error: Incomplete percent-encoded sequence"
    );
    assert_eq!(
        Address::try_from("tcp:opt=%1z").unwrap_err().to_string(),
        "Encoding error: Invalid hexadecimal character in percent-encoded sequence"
    );
    assert_eq!(
        Address::try_from("tcp:opt=1\rz").unwrap_err().to_string(),
        "Encoding error: Invalid character in address"
    );
    assert_eq!(
        Address::try_from("tcp:guid=9406e28972c595c590766c9564ce623")
            .unwrap_err()
            .to_string(),
        "Invalid value for key: `guid`"
    );
    assert_eq!(
        Address::try_from("tcp:guid=9406e28972c595c590766c9564ce623g")
            .unwrap_err()
            .to_string(),
        "Invalid value for key: `guid`"
    );

    let addr = Address::try_from("tcp:guid=9406e28972c595c590766c9564ce623f").unwrap();
    addr.guid().unwrap();
}
...
fn parse_unix() {
    let addr =
        Address::try_from("unix:path=/tmp/dbus-foo,guid=9406e28972c595c590766c9564ce623f").unwrap();
...
fn parse_launchd() {
    ...
    assert_eq!(
        Address::try_from("launchd:weof").unwrap_err().to_string(),
        "Missing key: `env`"
    );
}
...
#[test]
fn parse_tcp() {
    let addr = Address::try_from("tcp:host=localhost,bind=*,port=0,family=ipv4").unwrap();
    ...
    let addr = Address::try_from("tcp:").unwrap();
    let Transport::Tcp(t) = addr.transport().unwrap() else {
        panic!();
    };
...
fn parse_nonce_tcp() {
    let addr =
        Address::try_from("nonce-tcp:host=localhost,bind=*,port=0,family=ipv6,noncefile=foo")
            .unwrap();
...
#[cfg(feature = "vsock")]
fn parse_vsock() {
    let addr = Address::try_from("vsock:cid=12,port=32").unwrap();
    ...
    assert_eq!(
        Address::try_from("vsock:port=abc").unwrap_err().to_string(),
