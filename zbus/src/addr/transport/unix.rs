        ...
        for (k, v) in &mut iter {
            match k {
                ...
                "runtime" => {
                    ...
                    if v != "yes" {
                        return Err(Error::Address(format!("Invalid runtime={} value", v)));
                    }
        ...
        for (k, _) in iter {
            match k {
                "path" | "dir" | "tmpdir" | "abstract" | "runtime" => {
                    return Err(Error::Address("Invalid address: only one of `path` `dir` `tmpdir` `abstract` or `runtime` expected".into()));
                }
                _ => (),
            }
        }
