...
mod tests {
    ...
    #[test]
    fn u8_variant() {
        let encoded = to_bytes::<_, LE>(Format::DBus, &77_u8).unwrap();
        assert!(encoded.len() == 1);

        // As Variant
        let v = 77_u8.into_variant();
        assert!(v.value_signature() == "y");
        let encoded = to_bytes::<_, LE>(Format::DBus, &v).unwrap();
        assert!(encoded.len() == 4);
    }

    #[test]
    fn u16_variant() {
        let encoded = to_bytes::<_, BE>(Format::DBus, &0xABBA_u16).unwrap();
    ...
    #[test]
    fn i16_variant() {
        let encoded = to_bytes::<_, BE>(Format::DBus, &-0xAB0_i16).unwrap();
        assert!(encoded.len() == 2);
    ...
    #[test]
    fn u32_variant() {
        let encoded = to_bytes::<_, BE>(Format::DBus, &0xABBA_ABBA_u32).unwrap();
        assert!(encoded.len() == 4);
    ...
    #[test]
    fn i32_variant() {
        let encoded = to_bytes::<_, BE>(Format::DBus, &-0xABBA_AB0_i32).unwrap();
        assert!(encoded.len() == 4);
    ...
    #[test]
    fn u64_variant() {
        let encoded = to_bytes::<_, BE>(Format::DBus, &0xABBA_ABBA_ABBA_ABBA_u64).unwrap();
        assert!(encoded.len() == 8);
    ...
    #[test]
    fn i64_variant() {
        let encoded = to_bytes::<_, BE>(Format::DBus, &-0xABBA_ABBA_ABBA_AB0_i64).unwrap();
        assert!(encoded.len() == 8);
    ...
    #[test]
    fn f64_variant() {
        let encoded = to_bytes::<_, BE>(Format::DBus, &99999.99999_f64).unwrap();
        assert!(encoded.len() == 8);
    ...
    fn str_variant() {
        ...
        let encoded = to_bytes::<_, LE>(Format::DBus, &string).unwrap();
        assert!(encoded.len() == 16);
    ...
    fn signature_variant() {
        ...
        let encoded = to_bytes::<_, LE>(Format::DBus, &sig).unwrap();
    ...
    fn object_path_variant() {
        ...
        let encoded = to_bytes::<_, LE>(Format::DBus, &o).unwrap();
    ...
    fn array_variant() {
        ...
        let encoded = to_bytes::<_, LE>(Format::DBus, &ay[..]).unwrap();
        assert!(encoded.len() == 6);
        ...
        // Now try as Vec
        let vec = ay.to_vec();
        let encoded = to_bytes::<_, LE>(Format::DBus, &vec).unwrap();
        assert!(encoded.len() == 6);
        ...
        let encoded = to_bytes::<_, LE>(Format::DBus, &as_[..]).unwrap();
        ...
        let encoded = to_bytes::<_, LE>(Format::DBus, &ar[..]).unwrap();
    ...
    fn dict_variant() {
        ...
        map.insert(1, "123");
        map.insert(2, "456");
        let encoded = to_bytes::<_, LE>(Format::DBus, &map).unwrap();
