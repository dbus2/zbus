...
use std::{
    borrow::Cow,
    ffi::{CStr, OsStr, OsString, CString},
    path::{Path, PathBuf},
};
...
/// # Exmples
///
/// Consider using `from` to convert/cast [FilePath] to other compatible types, see the example bellow for reference
/// ```
...
impl<'f> From<CString> for FilePath<'f> {
...
impl<'f> From<&'f CString> for FilePath<'f> {
    fn from(value: &'f CString) -> Self {
        return FilePath(Cow::Borrowed(value.as_c_str()));
    }
}

impl<'f> From<&'f OsStr> for FilePath<'f> {
    fn from(value: &'f OsStr) -> FilePath<'f> {
        return FilePath(bytes_with_null(&value.as_encoded_bytes()));
    }
}
...
impl<'f> From<OsString> for FilePath<'f> {
    fn from(value: OsString) -> Self {
        return FilePath(vec_to_cstr(value.as_encoded_bytes().to_vec()));
    }
}
...
impl<'f> From<PathBuf> for FilePath<'f> {
    fn from(value: PathBuf) -> FilePath<'f> {
...
///
/// Truncates the vector at the first null byte, if present.
/// If no null byte exists, appends one to ensure proper termination.
///
/// # Returns
///
/// A `Cow<'_, CStr>` containing a *guaranteed* null-terminated string.
pub fn vec_to_cstr<'a>(mut bytes: Vec<u8>) -> Cow<'a, CStr> {
    if let Some(pos) = bytes.iter().position(|&b| b == 0) {
        bytes.truncate(pos + 1);
    } else {
        bytes.push(0);
    }
    // SAFETY: unwrap is fine here since we the null termination
    Cow::Owned(CString::from_vec_with_nul(bytes).unwrap())
}

/// Converts a byte slice into a null-terminated [CStr].
///
/// Returns a borrowed [CStr] if the slice already contains a null byte;
/// otherwise, returns an owned [CStr] with a null byte appended.
///
/// # Returns
///
/// A [Cow<'_, CStr>] containing a *guaranteed* null-terminated string.
fn bytes_with_null<'a>(bytes: &'a [u8]) -> Cow<'a, CStr> {
    // SAFETY: unwrap is fine here since in bot cases we guarante the null termination
    if let Some(pos) = bytes.iter().position(|&b| b == 0) {
        return Cow::Borrowed(CStr::from_bytes_with_nul(&bytes[..=pos]).unwrap());
    }
