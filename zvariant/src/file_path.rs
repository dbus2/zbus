...
/// # Exmples
///
/// Consider using `from` to convert/cast [FilePath] to other compatible types, see the example
...
fn bytes_with_null(bytes: &[u8]) -> Cow<'_, CStr> {
    ...
    //SAFETY unwrap is fine, as we handled the null containing case above
