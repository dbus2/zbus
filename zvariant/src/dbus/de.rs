...
impl<'de, 'f, F> Deserializer<'de, 'f, F> {
        ...
        match self.state {
            ...
            ValueAccessState::ReadingValue => {
                ...
                let mut de = Deserializer::new(
                    subslice(self.de.common.bytes, self.de.common.pos..)?,
                    #[cfg(unix)]
                    self.de.common.fds,
                    sig,
                    inner_context,
...
impl<'d, 'de, 'f, #[cfg(unix)] F: AsFd> SeqAccess<'de> for ArrayAccess<'d, 'de, 'f, F> {
        ...
        T: serde::de::DeserializeSeed<'de>,
    {
        if self.count > 0 {
            ...
            let result = seed.deserialize(&mut de).map(Some);
            self.de.common.pos += de.common.pos;
            self.count -= de.common.pos as i64;
            result
        } else {
            Ok(None)
        }
...
impl<'d, 'de, 'f, #[cfg(unix)] F: AsFd> VariantAccess<'de> for StructureEnumAccess<'d, 'de, 'f, F> {
        ...
        V: Visitor<'de>,
    {
        if let Some(SignatureEntry::Struct(fields)) = self.fields.pop_front() {
            ...
            visitor.visit_seq(StructureAccess::new(fields, self.de)?)
        } else {
            Err(crate::Error::MissingSignature)
        }
    }
