    ...
    W: Write + Seek,
{
    ...
    {
        let inner_context = Context::new_dbus(
            self.ser.common.ctxt.endian(),
            self.ser.common.ctxt.position() + self.ser.common.bytes_written,
        );

        let mut ser = Serializer::new(
            self.signature.clone(),
            self.ser.common.writer,
            self.ser.common.fds,
            inner_context,
        )?;
        value.serialize(&mut ser)?;
        self.ser.common.bytes_written += ser.common.bytes_written;
        self.len += ser.common.bytes_written as i64;
        ...
        match name {
            Some("zvariant::Value::Signature") => {
                ...
                if let Value::Signature(signature) = parsed_value {
                    ...
                    let result = signature.as_str().serialize(&mut ser);
                    self.ser.common.bytes_written += ser.common.bytes_written;
                    result
                } else {
